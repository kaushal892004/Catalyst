...
app = Flask(__name__)
app.secret_key = 'bfb04044e0d800ef68fef96b656c0b5c18cfba233f16c402566d9118bd6c1e3f'
...
#     # Example: mongo.db.users.update_one({'email': email}, {'$set': {'full_name': full_name, ...}})
...
# Profile route to render the profile page
...
#         # If the user is found in the database, render the profile page with the user's data
#         if user:
#             return render_template('profile.html', user=user)
#         else:
#             # If user data is not found in the database, redirect to login
#             return redirect(url_for('login'))
#     else:
...
#                 # Save the profile picture and update the user record
#                 profile_picture.save(f'static/profile_pics/{session["user_id"]}.png')
#                 db.users.update_one({'_id': ObjectId(session['user_id'])}, {'$set': {'profile_pic': f'{session["user_id"]}.png'}})
...
# Define a folder to save profile pictures
UPLOAD_FOLDER = 'static/images/profile_pics'  # Adjust as necessary
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif'}
...
def profile():
    if request.method == 'POST':
        ...
        if profile_picture and allowed_file(profile_picture.filename):
            ...
            profile_picture_url = None  # Handle case if no file is uploaded

        # Here you can save the data to your database
        # Example: save_profile_to_db(full_name, current_position, industry, phone_number, email, linkedin, profile_picture_url)
...
@login_required  # Ensure the user is logged in
def upload_profile_pic():
    # Check if the 'profile_picture' field exists in the form submission
    if 'profile_picture' in request.files:
        profile_picture = request.files['profile_picture']
        
        # Make sure the user has selected a picture
        if profile_picture.filename != '':
            # Save the profile picture to the 'static/profile_pics/' directory
            picture_path = os.path.join('static/profile_pics', f'{current_user.id}.png')
            profile_picture.save(picture_path)
            
            # Update the user record in MongoDB to store the picture filename
            db.users.update_one(
                {'_id': ObjectId(current_user.id)},
                {'$set': {'profile_pic': f'{current_user.id}.png'}}
            )
...
# ------------------------------------ #
# User login route with bcrypt and Flask-Login
# ------------------------------------ #
@app.route('/login', methods=['GET', 'POST'])
def login():
    if current_user.is_authenticated:
        return redirect(url_for('landing_page'))  # Redirect if already logged in
...
@login_required  # Ensure the user is logged in
def logout():
    logout_user()  # Use Flask-Login's logout function to log out the user
